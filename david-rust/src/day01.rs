...
fn get_nums(input: CachedStringInput) -> Result<Vec<i64>> {
...
pub fn adjust_frequency(input: CachedStringInput) -> PuzzleResult {
  let result = (get_nums(input)?).iter().sum();
  Ok(PuzzleSolution::Day01(result))
}

pub fn find_duplicate_frequency(input: CachedStringInput) -> PuzzleResult {
  let mut result = 0;
